...
from Keyence_PLC_RIKAZAI.ref_lib import kvhostlink
...
class PLC_Kyenece():
    ...
    def connect(self):
        """PLCに接続を開始するメソッド"""
        try:
            print(f"{self.ip_address}:{self.port} への接続を開始します...")
            # initialize the connection
            self.kv = kvhostlink.kvHostLink(host=self.ip_address, port=self.port)
            print("PLCに接続しました。")

            self._set_initial()

        except Exception as e:
            print(f"PLCに接続できませんでした: {e}")
            self.kv = None
    ...
    def disconnect(self):
        ...
        try:
            if self.kv is not None:
                print(f"{self.ip_address}:{self.port} への接続を切断します...")
                # 実際の切断処理をここに実装...
                # kvhostlinkには明確な切断メソッドが提供されていない可能性があるので、
                # 必要に応じて適切な処理を追加してください。
                self.kv = None
                print("PLCの接続を切断しました。")
            else:
                print("既にPLCとの接続は切断されています。")
        except Exception as e:
            print(f"PLCの接続を切断できませんでした: {e}")
    ...
    def set_x_speed(self, speed_x):
        # 入力速度を所望の単位に変換 (10倍) して、PLCの期待するフォーマットに合わせる
    ...
    def set_x_acc(self, acc_x):
        # 入力速度を所望の単位に変換 (100倍) して、PLCの期待するフォーマットに合わせる
    ...
    # JOG実行指令 OFF
    def _jog_command_execute_off(self):
    ...
    def stop_jog_move(self):
        # 4.コマンド実行指令OFFを入れること。
        self._jog_command_execute_off()


    '''
    Tkinter Button Binding
    class YourClass:
    # ... その他の部分 ...

    def start_move_x_jog_plus(self):
        # 0. コマンド実行状態のOFFを確認する
        self._check_jog_command_execute_state_is_off()
        # 1. コマンド No.指定
        self.kv.write('W1002', str(12))  # 12: カメラX軸 JOG+移動
    ...
    def stop_move_x_jog_plus(self):
        self._jog_command_execute_off()
        print('Jog Stopped')

    def create_gui(self):
        root = tk.Tk()
        button = tk.Button(root, text="JOG+")
        button.pack()
        button.bind('<Button-1>', lambda event: self.start_move_x_jog_plus())
        button.bind('<ButtonRelease-1>', lambda event: self.stop_move_x_jog_plus())
        root.mainloop()
