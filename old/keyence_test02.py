...
def move_robot(axis=1, loc=0.0):
    ...
    if axis == 1:
        ...
        while True:
            ...
            # Removing the unnecessary characters and extracting the desired string
            clean_data = decoded_data.strip('\r\n')
            print(f'2nd **check command state, {clean_data}')
            if clean_data == '00000':
                break
...
def move_jog_x(axis=1):
    if axis == 1:
        # 1.コマンド No.指定
        # X軸 + jog
        data = kv.write('W1002', '12')

        # 3.コマンド実行指令 ON
        data = kv.write('W1004', str(1))

def move_jog_minusx(axis=1):
    if axis == 1:
        # 1.コマンド No.指定
        # X軸 + jog
        data = kv.write('W1002', '13')
...
if __name__ == "__main__":
    ...
    time.sleep(1)
    # 6. コマンド実行指令 OFF
    data = kv.write('W1004', str(0))

    move_jog_minusx(axis=1)
    time.sleep(1)
    # 6. コマンド実行指令 OFF
    data = kv.write('W1004', str(0))


    # data= move_robot(axis=1, loc=300)
    ...
    # # time.sleep(1)
    # #
    # data2 = move_robot(axis=2, loc=300)
    #
    # print(f'data2 :{data2}')
    #
    # # data = move_reset()
